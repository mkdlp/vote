...
drop sequence seq_ooption
create sequence seq_ooption start with 450
...
delete from VoteItem where viid between 500 and 505


select vsId,vsTitle,
		(select count(vsId) from VoteOption where vsId=s.vsId) optionCount,
		(select count(vsId) from VoteItem where vsId=s.vsId) voteUserCount from VoteSubject s


select count(i.voId) from VoteOption o
		left join VoteItem i on o.voId=i.voId where o.voId=103 group by voorder order by voorder


select *from VoteOption where vsid=103
